...
def add_features(inputfile):
    ...
    raw_tokens = df['Token'].tolist()
    
    # preprocess tokens (remove trailing punctuation)
    tokenlist = []
    for token in raw_tokens:
        token = str(token)
        if token.endswith('.') or token.endswith(','):
            newtoken = token.rstrip('.,')
            tokenlist.append(newtoken)
        else:
            tokenlist.append(token)

    # convert labels and put them into a list
    ...
    for label in labels:
        if label == '_':
            label_list.append('O')
        elif label == 'B-negation':
            label_list.append('B-NEG')
        elif label == 'I-negation':
            label_list.append('I-NEG')
            
    # update label list in dataframe
    df['Label'] = label_list
...
# function call
inputfile = 'data/bioscope.clinical.columns.txt'
add_features(inputfile)
